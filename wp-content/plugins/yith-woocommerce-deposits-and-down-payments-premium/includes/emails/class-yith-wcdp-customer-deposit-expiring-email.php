	...
	class YITH_WCDP_Customer_Deposit_Expiring_Email extends YITH_WCDP_Emails {
		...
		public function __construct() {
			...
			$this->content_html = $this->get_option(
				'content_html',
				__(
					'<p>Hurry up! The following down payments are expiring: you have only <strong>{days_before_expire}</strong> days left</p>
					{deposit_list}',
			...
			$this->content_text = $this->get_option(
				'content_text',
				__(
					"Hurry up! The following down payments are expiring: you have only {days_before_expire} days left\n
					{deposit_list}",
					'yith-woocommerce-deposits-and-down-payments'
				)
			);
		...
		public function trigger( $order_id, $suborder_id = false, $force = false ) {
			...
			$this->suborders   = YITH_WCDP_Suborders()->get_suborder( $order_id );
		...
		public function is_enabled() {
			...
			return apply_filters( 'yith_wcdp_is_email_enable', ( 'yes' === $deposit_expire && 'yes' === $notify_customer ), $deposit_expire, $notify_customer );
		}
