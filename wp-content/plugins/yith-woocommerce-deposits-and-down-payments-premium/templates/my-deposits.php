			...
			<?php foreach ( $deposits as $deposit ) : ?>
				<tr>
					...
					<td class="order-actions" data-title="<?php esc_html_e( 'Actions', 'yith-woocommerce-deposits-and-down-payments' ); ?>">
						<?php
						if ( $deposit['actions'] ) {
							foreach ( $deposit['actions'] as $key => $deposit_action ) {
								echo '<a rel="nofollow" href="' . esc_url( $deposit_action['url'] ) . '" class="button ' . sanitize_html_class( $key ) . '">' . esc_html( $deposit_action['name'] ) . '</a>';
							}
						}
						?>
					</td>
